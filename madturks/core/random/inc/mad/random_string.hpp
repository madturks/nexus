 ...
 */
[[nodiscard]] auto
ascii_alphanumeric_charset() -> const std::span<const char> &;
...
generate(std::size_t min_l = 16, std::size_t max_l = 256,
         const std::span<const char> & charset = ascii_alphanumeric_charset())
    -> std::string {
    std::string v{};
...
inline auto fill(Iter begin, Iter end, std::size_t min_length = 16,
 ...
 * @param [in] min_length (optional) Minimum length of generated strings
 * (default=16)
 * @param [in] max_length (optional) Maximum length of generated strings
 * (default=256)
 *
...
requires(std::is_same_v<std::string, ValueType> && has_push_back<Container> &&
         std::is_default_constructible_v<Container>)
[[nodiscard]] inline auto make(std::size_t count, std::size_t min_length = 16,
                               std::size_t max_length = 256) {
    Container container;
...
requires(std::is_same_v<std::string, ValueType> &&
         ...
         std::is_default_constructible_v<Container>)
[[nodiscard]]
inline auto make(std::size_t count, std::size_t min_length = 16zu,
                 std::size_t max_length = 256zu) {
    Container container;
    while (count) {
        while (container.insert(generate<ValueType>(min_length, max_length))
                   .second == false)
            ;
        count--;
    }
