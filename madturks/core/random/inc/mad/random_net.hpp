...
namespace mad {
    ...
    requires(std::is_same_v<net::ip::address_v4, ValueType> || std::is_same_v<net::ip::address_v6, ValueType>)
    ...
    /**
     * Make a container filled with random `net::ip::address_v4` or `net::ip::address_v6`
     * elements.
     *
     * (.push_back() overload)
     *
    ...
    template <typename Container, typename ValueType = typename Container::value_type>
    requires((std::is_same_v<net::ip::address_v4, ValueType> || std::is_same_v<net::ip::address_v6, ValueType>) &&
             has_unary_insert<Container> && std::is_default_constructible_v<Container>)
    NCF_NODISCARD inline auto make(std::size_t count) {
        Container container;
        while (count) {
