 ...
 *
 * @param [in] lower_bound Lower bound of generated value. Default:
 * `std::numeric_limits<T>::min()`
 * @param [in] upper_bound Upper bound of generated value. Default:
...
generate(T lower_bound = std::numeric_limits<T>::min(),
         T upper_bound = std::numeric_limits<T>::max()) -> T {
    T v{};
    std::span<T> result_span = { &v, 1 };
    fill_span(result_span, arithmetic_boundary<T>{ .lower = lower_bound,
                                                   .upper = upper_bound });
    return v;
}
...
fill(Iter begin, Iter end,
     ValueType lower_bound = std::numeric_limits<ValueType>::min(),
...
inline auto fill(Iter begin, Iter end,
                 typename Iter::value_type lower_bound =
                     std::numeric_limits<typename Iter::value_type>::min(),
                 typename Iter::value_type upper_bound =
                     std::numeric_limits<typename Iter::value_type>::max()) {
    while (begin != end) {
        *begin++ = generate<typename Iter::value_type>(
...
template <typename Container,
          ...
          typename BoundType>
requires(std::is_arithmetic_v<ValueType> && has_push_back<Container> &&
         std::is_default_constructible_v<Container>)
[[nodiscard]] inline auto
make(std::size_t count,
     BoundType lower_bound = std::numeric_limits<ValueType>::min(),
...
template <typename Container,
          typename ValueType = typename Container::value_type,
          typename BoundType>
requires(std::is_arithmetic_v<ValueType> && has_unary_insert<Container> &&
         std::is_default_constructible_v<Container>)
[[nodiscard]] inline auto
make(std::size_t count,
     BoundType lower_bound = std::numeric_limits<ValueType>::min(),
    ...
    Container container;
    while (count) {
        while (container.insert(generate<ValueType>(lower_bound, upper_bound))
                   .second == false)
            ;
        count--;
    }
