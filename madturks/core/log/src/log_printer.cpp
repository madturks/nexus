...
void log_printer::load_configuration_file(
    ...
    spdlog_setup::from_file_and_override(
        ...
        std::string{ override_configuration_file });
    // LCOV_EXCL_STOP
}

void log_printer::log_impl(log_level level,
                           const mad::meta::source_location & sloc,
    ...
    spdlog::details::log_msg log_msg{
        spdlog_sloc, logger->name(),
        static_cast<spdlog::level::level_enum>(level), message
    };
    // (mgilor): To avoid spdlog internal level check and such, call sink_it
    // directly
...
void log_printer::set_log_level(log_level level) {
    assert(logger_instance);
    try {
        static_cast<spdlog::logger *>(logger_instance.get())
