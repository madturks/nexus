...
using MyTypes =
    ::testing::Types<mad::circular_buffer_vm<vm_cb_backend_shm>, mad::circular_buffer_vm<vm_cb_backend_mmap>, mad::circular_buffer>;

class NameGenerator {
public:
    template <typename T>
    static std::string GetName(int) {
        if constexpr (std::is_same_v<T, mad::circular_buffer_vm<vm_cb_backend_shm>>)
            return "shm";
        if constexpr (std::is_same_v<T, mad::circular_buffer_vm<vm_cb_backend_mmap>>)
            return "mmap";
...
TYPED_TEST(cb_fast_fixture, push_full) {
    ...
    // Check we cannot push any more data (which would override unprocessed data)
...
TYPED_TEST(cb_fast_fixture, push_wrap_around) {
    ...
    EXPECT_TRUE(alloc.put_all(put_bufs [3], put_bufs [2])); // this will wrap around.
    EXPECT_TRUE(alloc.get_all(get_bufs [0], get_bufs [1], get_bufs [2], get_bufs [3]));

    EXPECT_TRUE(std::equal(get_bufs [0].begin(), get_bufs [0].end(), put_bufs [2].begin()));
    EXPECT_TRUE(std::equal(get_bufs [1].begin(), get_bufs [1].end(), put_bufs [3].begin()));
    EXPECT_TRUE(std::equal(get_bufs [2].begin(), get_bufs [2].end(), put_bufs [3].begin()));
    EXPECT_TRUE(std::equal(get_bufs [3].begin(), get_bufs [3].end(), put_bufs [2].begin()));
}
...
TYPED_TEST(cb_fast_fixture, TransferData) {
    if constexpr (std::is_same_v<TypeParam, mad::circular_buffer_vm<vm_cb_backend_mmap>> ||
                  std::is_same_v<TypeParam, mad::circular_buffer_vm<vm_cb_backend_shm>>) {
        TypeParam buffer_{page_size};
...
TYPED_TEST(cb_fast_fixture, AutoAlignToPage) {
    ...
    if constexpr (std::is_same_v<TypeParam, mad::circular_buffer_vm<vm_cb_backend_mmap>> ||
                  std::is_same_v<TypeParam, mad::circular_buffer_vm<vm_cb_backend_shm>>) {
        TypeParam aligned_buffer(1024, typename TypeParam::auto_align_to_page{});

        // Verify alignment to the page size
        EXPECT_EQ(aligned_buffer.empty_space() % static_cast<std::size_t>(getpagesize()), 0);
...
TYPED_TEST(cb_fast_fixture, OverflowUnderflowStress) {
    ...
    std::vector<std::uint8_t> large_data(page_size + 100, 'O'); // Intentionally larger than buffer
    for (int i = 0; i < 500; ++i) {
        EXPECT_FALSE(buffer_.put(large_data.data(), large_data.size())); // Should fail due to overflow
    }

    std::array<std::uint8_t, 1> small_data;
    for (int i = 0; i < 500; ++i) {
        EXPECT_FALSE(buffer_.get(small_data.data(), small_data.size())); // Should fail due to underflow
    }
}
