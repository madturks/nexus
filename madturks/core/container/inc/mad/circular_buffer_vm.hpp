 ...
 * In normal scenario, we allocate a linear, contiguous chunk of memory for our
 * circular buffer and add some code to use it as if it was a circular buffer.
 * To do this, we calculate the amount of data free from front of the buffer and
 * the end of the buffer seperately, then split the data we're going to write
 * between these two memory areas(I call it as region). This costs around two
 * memcpy calls in worst scenario.
 *
 * But what if the buffer we've allocated was indeed a non-linear, circular
 * buffer? The trick you're going to see below, exactly simulates this by using
 * a memory mapping trick.
 *
 * I'm not going to dive in much details of how operating system's memory
 * management works, but it manages the physical memory by dividing it to pages.
 * This pages are held in physical page table. When a process request some
 * memory from the operating system, some pages are dedicated to the process.
 *
 * After the starting of multitasking era, memory fragmentation become a real
 * issue, because every single process was using the same page table. This
 * caused gaps between free pages, causing some allocations fail even if we had
 * enough pages to provide the requested memory.
 *
 * Thus, the UNIX developers introduced `virtual page`, assigned to each process
 * independently. In its' core, this allowed out-of-order physical page mapping
 * on the virtual page.
 *
 * Assume that we've got the following physical page table, and two virtual page
 * tables for each;
 *
 ...
 *
 * Process 1 now request 4 contiguous pages from the operating system, and the
 * operating system happily give it.
 *
 * PHYSICAL PAGE TABLE      PROCESS 1 VIRTUAL PT        PROCESS 2 VIRTUAL PT
 ...
 *
 * As for point of view of process 2, the pages are contiguous, but in reality,
 * they're seperated by half in physical memory. If we tried the scenario above
 * without the virtual page table, the operating system will return nothing for
 * process 2's request, because it would lack thre required amount of adjacent
 * pages.
 *
 * (**I said briefly, but explained throughly..)
 *
 * The following hack enables us to exploit the virtual table of our process. If
 * we go back the first circular buffer scenario, we had to wrap around the
 * buffer and calculate the amount of free space from beginning to head, from
 * tail to end and put our data there. This costs two memcpy calls. In the code
 * below, we allocate an anonymous file in the memory, and map two of our
 * adjacent page blocks to it. Thus, when we overflow our index, we actually
 * wrap around the underlying anonymous file. Therefore, we can write and read
 * data from our buffer with a single memcpy call.
 *
 * Pretty neat trick.
 *
 *
 * @tparam cb_backend_shm Type of the backend to use perform sequential virtual
 * page mapping
 */
...
struct circular_buffer_vm : public circular_buffer_base {
     ...
     * Enables automatic page size alignment for circular buffer fast
     * constructor. Assuming the page size is 1024 bytes and requested amount is
     * eg. 1017 bytes, the constructor will round it up to nearest multiple of
     * 1024, which is 1024. This is neccessary to this circular buffer
     ...
     *
     * @param    buffer     Pointer to buffer which contains the content to be
     * put
    ...
    /**
     * @brief Peek `Size` byte(s) from circular buffer into given std::array
     * (`buffer`)
     *
    ...
    std::span<const std::uint8_t> available_span() const {
        return std::span<const std::uint8_t>{ native_buffer_.get() + head_,
                                              consumed_space() };
    }
    ...
    auto peek_assign(T & dst) const -> bool {
        ...
        dst.assign(native_buffer_.get() + head_,
                   native_buffer_.get() + head_ + consumed_space());
        return true;
    }
     ...
     * then only the amount that can fit will be transferred to the other
     * buffer, rest will still be in this buffer.
