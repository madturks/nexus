...
circular_buffer_vm<BT>::circular_buffer_vm(const size_t size) :
    ...
    if constexpr (cb_has_shm_backend<BT>) {
         ...
         */
        vaddr = static_cast<element_t *>(
            mmap(NULL, 2 * size, PROT_READ | PROT_WRITE,
        ...
        if (vaddr + size != shmat(shm_id, vaddr + size, 0)) {
            ...
            shmctl(shm_id, IPC_RMID, nullptr);
            throw std::runtime_error{
                "Failed to attach shared memory segment at second address"
            };
        }
        ...
        if (shmctl(shm_id, IPC_RMID, nullptr) < 0) {
            ...
            shmdt(vaddr + size);
            throw std::runtime_error{
                "Failed to mark shared memory segment for removal"
            };
        }
        ...
        /**
         * @brief uuid
         * Anonymous files require a name, and we might allocate a dozen of
         * them. Therefore the name should be unique in order to prevent
         * collisions.
         */
         ...
         * Get an address for allocation.
         */
        vaddr = static_cast<element_t *>(mmap(
            NULL, 2 * size, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
         ...
         * */
        if (mmap(vaddr, size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,
                 anonymous_fd_, 0) == MAP_FAILED)
            throw std::runtime_error{
                "Failed to map first page of memory to anonymous file"
            };
        /*
         * Map the second page of the memory to our anonymous file
         * */
        if (mmap(vaddr + size, size, PROT_READ | PROT_WRITE,
                 MAP_SHARED | MAP_FIXED, anonymous_fd_, 0) == MAP_FAILED)
            throw std::runtime_error{
                "Failed to map second page of memory to anonymous file"
            };
    }
...
static constexpr auto round_up_to_multiple(std::size_t value,
...
circular_buffer_vm<BT>::circular_buffer_vm(const size_t size,
                                           auto_align_to_page) :
    circular_buffer_vm(
        round_up_to_multiple(size, static_cast<std::size_t>(getpagesize()))) {}

template <circular_buffer_backend BT>
circular_buffer_vm<BT>::circular_buffer_vm(circular_buffer_vm &&) = default;

template <circular_buffer_backend BT>
circular_buffer_vm<BT> &
circular_buffer_vm<BT>::operator=(circular_buffer_vm &&) = default;
...
template <circular_buffer_backend BT>
auto circular_buffer_vm<BT>::put(const element_t * buffer,
                                 const size_t size) -> bool {
    // Prevent overcommit
    if (empty_space() < size) [[unlikely]]
        return false;

    // In this version, we don't need to worry about adjusting head region and
    // tail region as we mapped the adjacent region of memory to same physical
    // memory. Therefore, memcpy call will automatically wrap around the buffer.
...
template <circular_buffer_backend BT>
auto circular_buffer_vm<BT>::peek(element_t * dst,
                                  const size_t amount) const -> bool {
...
auto circular_buffer_vm<BT>::transfer(circular_buffer_vm & other) -> void {
    ...
    const size_t transfer_amount = (consumed_space() > other.empty_space()
                                        ? other.empty_space()
                                        : consumed_space());
...
// The thing with explicit template instantiation is, it must be in same
// translation unit with template class definition. For me, it's absolute
// bullshit.
