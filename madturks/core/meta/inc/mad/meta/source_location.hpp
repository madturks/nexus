...
struct source_location {
     ...
     * @param [in] function_name_size Size of source location function name
     * string
     * @param [in] cline Source line index on file
     */
    inline consteval source_location(
        const char * file_name = __builtin_FILE(),
        int file_name_size = __builtin_strlen(__builtin_FILE()),
        const char * function_name = __builtin_FUNCTION(),
        int function_name_size = __builtin_strlen(__builtin_FUNCTION()),
        int cline = __builtin_LINE()) {
