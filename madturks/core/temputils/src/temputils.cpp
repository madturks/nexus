...
namespace mad {
    ...
    std::optional<std::filesystem::path> make_temp_file_path(std::string_view pattern) noexcept {
        try {
    ...
    // Make temporary file
    std::optional<std::pair<std::filesystem::path, std::ofstream>> make_temp_file(std::string_view pattern) noexcept {
        try {
            if (auto path = make_temp_file_path(pattern)) {
                if (std::ofstream ofs(path.value(), std::ios::out | std::ios::trunc); ofs.is_open()) {
